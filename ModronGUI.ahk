...
;Intall locations
global strSTMpath := "C:\Program Files (x86)\Steam\steamapps\common\IdleChampions\IdleDragons.exe"
...
;Enable servercalls to open chests during stack restart
...
global wTitle := "Mike's GemFarmer Modron for EGS (" . ScriptVersion . ")"
LogFMsg("VERSION INFO: ModronGUI.ahk - " . wTitle)
LogMsg( "VERSION INFO: ModronGUI.ahk - " . wTitle, true)
LogMsg("VERSION INFO: IC_Memoryfunctions.ahk (" . MF_ScriptVersion . ")", true)
...
Gui, MyWindow:Add, Text, x15 y+40, Instructions:
...
; // GUITAB: Settings /////////////////////////////////////////////////////////////////////////////
Gui, Tab, Settings
Gui, MyWindow:Add, Text, x15 y80 w220, Seats to level with Fkeys:
Loop, 12
{
    i := gSeatToggle[A_Index]
    if (A_Index = 1)
    Gui, MyWindow:Add, Checkbox, vCheckboxSeat%A_Index% Checked%i% x15 y+5 w70, Seat %A_Index%
    Else if (A_Index <= 6)
    Gui, MyWindow:Add, Checkbox, vCheckboxSeat%A_Index% Checked%i% x+5 w70, Seat %A_Index%
    Else if (A_Index = 7)
    Gui, MyWindow:Add, Checkbox, vCheckboxSeat%A_Index% Checked%i% x15 y+5 w70, Seat %A_Index%
    Else
    Gui, MyWindow:Add, Checkbox, vCheckboxSeat%A_Index% Checked%i% x+5 w70, Seat %A_Index%
...
Gui, MyWindow:Add, Text, x+5 w300, %strGUI%
;----------------- <HOTKEY
...
Gui, Tab, Help
;Gui, MyWindow:Font, w700
Gui, MyWindow:Add, Text, x15 y80, Confirm your settings are saved. 
...
Gui, Tab, Stats
Gui, MyWindow:Font, w700
Gui, MyWindow:Add, Text, x15 y83, Stats updated continuously (mostly):
Gui, MyWindow:Font, w400
...
Gui, MyWindow:Font, cSilver s11 ;
Gui, MyWindow:Font, w700
Gui, MyWindow:Add, Text, x15 y85, Timers:
Gui, MyWindow:Font, w400
...
CloseIC()
{
    PostMessage, 0x112, 0xF060,,, ahk_exe IdleDragons.exe   ; TODO: document what message is this sending to the IC executable?
    start := A_TickCount
...
SetFormation(zoneNum)
{
    ...
    else if (!ReadQuestRemaining(1) AND ReadTransitioning(1) AND zoneNum < gAreaLow)
    {
...
LoadingZoneREV()
{
    ...
    while (ReadChampBenchedByID(1,, 58) != 1 AND UpdateElapsedTime(start) < 60000)
    {
    ...
    while (ReadChampBenchedByID(1,, 58) != 0 AND UpdateElapsedTime(start) < 30000)
    {
        DirectedInput("w{F5}w")
        UpdateStatTimers()
    }
    ;check if stuck function would fail here on some cases where game gets stuck in offline progress calc, common after invalid instance. memory would read as if progress was still happening.
    if (UpdateElapsedTime(start) > 30000)
    {
        CloseIC()
        Sleep, 1000
        SafetyCheck()
...
{  
    ...
    GuiControl, %GUIwindow%, ReadUserHashID, % ReadUserHash()
    advtoload := ReadCurrentObjID(0)
    GuiControl, MyWindow:, advtoloadID, % advtoload
    if CheckSetUpREV()
       Return
...
StuffToSpam(SendRight := 1, zoneNum := 1, hew := 1, formation := "")
{
    ...
    if (gCtrlClickLeveling)
        ...
        var := var "``"
    if (gContinuedLeveling > zoneNum)
        var := var g_FKeys    ;var := BuildLevelKeys(zoneNum) ;LJ gFKeys
    if (gHewUlt AND hew)
        var := var gHewUlt
